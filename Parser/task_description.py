...
class TaskDescription:
    ...
    def get_task_list(self):
        ...
        try:
            ...
            print("Parsing failed, because of: " + str(e))
        return task_list
    ...
    def get_name(self, token):
        if token.is_name():
            return token.value
        raise Exception(f"Expected TokenType.NAME, got {token}")

    def get_duration(self):
        token = self.tokenizer.next_token()
        if not token.is_takes():
            raise Exception(f"Expected TokenType.TAKES, got {token.value}")
        token = self.tokenizer.next_token()
        if not token.is_number():
            raise Exception(f"Expected TokenType.NUMBER, got {token.value}")
        return int(token.value)

    def get_dependencies(self):
        token = self.tokenizer.next_token()
        if not token.is_needs():
            raise Exception(f"Expected TokenType.NEEDS, got {token.type}")
        else:
    ...
    def evaluate_dependencies(self, dependencies):
        ...
        if count_open_brackets != count_close_brackets:
            raise Exception("Invalid syntax, missing brackets")
